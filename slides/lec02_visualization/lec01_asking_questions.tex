...
\begin{itemize}
\item visualize economic inequality with\\graphs that summarize survey data
...
\node[anchor = north, font = \Large, align = left] at (.5,.5) {\bblue{Lecture 1: Asking Research Questions}};
\end{tikzpicture}
\end{frame}
...
\Huge
\bref{https://soc114.github.io/}{soc114.github.io}
\end{frame}
...
\draw[line width = 1.5pt, line cap = round] (goal.south west) -- (goal.south east);
\node[anchor = west] at (0, .75) {What is the average causal effect of motherhood on employment};
\node[anchor = west] at (0, .68) {among U.S. resident women ages 21--35?};
\only<2->{
% Title rows
...
\begin{frame}{Language for descriptive and causal questions}
...
\begin{frame}{Example: Effect of public housing on eviction}{Lundberg et al. \bref{https://doi.org/10.1002/pam.22234}{2021}}
...
if they had lived in a private rental? \vskip .1in
...
\item<2-> target population
\begin{itemize}
\item<4-> children born in large U.S. cities in 1998--2000\\who lived in public housing at age 9
\end{itemize}
\item<2-> outcome
\begin{itemize}
\item<5-> evicted from home between age 9 and 15
\end{itemize}
\end{itemize}
...
among kids who \bblue{looked like them}\\
but who live in a private rental \vskip .2in \pause
We \bblue{assumed} those rates would have happened\\to our target population in the absence of public housing
\end{frame}

\begin{frame}{Effect of \bblue{public housing} on \bblue{eviction}}
\centering
\begin{tikzpicture}
\node at (0,0) {\includegraphics[height = .8\textheight]{effect_public_evicted_3_}};
\end{tikzpicture}
\end{frame}
