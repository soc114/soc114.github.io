...
\begin{itemize}
\item estimate average causal effects with a parametric model
\begin{itemize}
\item for the outcome $\E(Y\mid A,\vec{L})$
\item for the treatment $\P(A\mid\vec{L})$
\end{itemize}
...
For every unit $i$,
\begin{itemize}
\item find units who look like them on confounders $\vec{L}$
\item who actually got treatment $A = a$
\item take the average among those units
\end{itemize}
...
\node (a) at (1,0) {$A$};
\node (y) at (2,0) {$Y$};
\draw<1>[->, line width = 2pt, blue] (l) -- (a);
\draw<1-2>[->, thick] (a) -- (y);
\draw[->, thick] (l) to[bend left] (y);
\end{tikzpicture}
\end{center}
