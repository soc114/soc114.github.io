...
\bgray{Solution:} DAG + estimation \pause
$$\frac{1}{n_1}\sum_{i:A_i=1}\left(Y_i^1 - \E\left(Y\mid A = 0, \vec{X} = \vec{x}_i\right)\right)$$ \pause
\bgray{Matching:} Estimate $\E(Y\mid A = 0, \vec{X} = \vec{x}_i)$ from one or more\\\hspace{52pt} untreated units with $\vec{X}$ ``near'' $\vec{x}_i$ \pause \vskip .1in
...
\draw[->, thick] (0,-1) -- node[midway, below] {Confounder $\vec{X}$} (1,-1);
\draw<2->[dashed] (p1) -- (p3);
...
\begin{frame}{What if $\vec{X}$ is multivariate?}
...
\begin{frame}{A common distance metric: Propensity scores}
Propensity score: $\pi_i = \P(A = 1\mid \vec{X} = \vec{x}_i)$ \pause
\begin{itemize}
\item Univariate summary of all confounders \pause
\item In expectation, a sample balanced on $\pi$ is balanced on $\vec{X}$
\begin{itemize}
...
\begin{itemize} \pause
\item Nonparametric: $\hat\pi_i$ is the proportion treated in the sample stratum $\vec{X} = \vec{x}_i$. \pause
\item Parametric: Often estimated as
\begin{itemize}
\item Fit logistic regression
$$\text{logit}\left(\P(A = 1\mid \vec{X})\right) = \alpha + \vec{X}\vec\beta$$ \pause
\item Predict the probability of treatment
$$\hat\pi_i = \text{logit}^{-1}\left(\hat\alpha + \vec{x}_i\hat{\vec\beta}\right)$$ \vspace{-.2in}
\end{itemize}
\end{itemize} \pause
...
\begin{itemize}
\item Multivariate $\vec{X}_i$ to univariate $\pi_i$ \pause
\begin{itemize}
...
Why we match: So $\vec{X}$ follows a similar distribution
\begin{itemize}
...
\begin{itemize}
\item Compare means of $\vec{X}$ across groups
...
$$d(i,j) = \sqrt{\left(\vec{X}_i - \vec{X}_j\right)^T\Sigma^{-1}\left(\vec{X}_i - \vec{X}_j\right)}$$
where $\Sigma = \V(\vec{X})$, the variance-covariance matrix
\end{frame}
...
$$
d(i,j) = \begin{cases}
0 &\text{if }\vec{X}_i = \vec{X}_j \\
\infty &\text{if }\vec{X}_i \neq \vec{X}_j \\
\end{cases}
$$
...
\pause
\begin{itemize}
\item Define $\tilde{\vec{X}}$ to be a coarsened version of $\vec{X}$ \pause
\begin{itemize}
\item Example: Convert all continuous values to decile categories
\end{itemize} \pause
\item Match exactly on $\tilde{\vec{X}}$
\end{itemize} \pause \vskip .1in
$$
d(i,j) = \begin{cases}
0 &\text{if }\tilde{\vec{X}}_i = \tilde{\vec{X}}_j \\
\infty &\text{if }\tilde{\vec{X}}_i \neq \tilde{\vec{X}}_j \\
\end{cases}
$$
\begin{itemize} \pause
\item Benefit: Very transparent \pause
\item Drawback: Ignores $\vec{X}$ within $\tilde{\vec{X}}$ \pause
...
When matching on multivariate $\vec{X}$, you have to define the distance between each pair of confounder values $\vec{x}$ and $\vec{x}'$
\begin{itemize}
...
\begin{itemize}
\item First, stratify units on $\vec{X}$ \pause
\item Then, randomize $A$ within blocks by a coin flip \pause
\item Imbalance on $\vec{X}$ is zero in every sample \pause
\end{itemize}
\item Experimental ideal 2: Completely randomized experiment \pause
\begin{itemize}
\item Assign each unit a known $\pi_i = \P(A = 1\mid \vec{X} = \vec{x}_i)$ \pause
\item Randomize treatment weighted by $\pi_i$ \pause
\item Imbalance on $\vec{X}$ is zero in expectation,\\but not in any particular sample \pause
\end{itemize}
\end{itemize}
...
King \& Nielsen claim: Better to match on $\vec{X}$ directly
