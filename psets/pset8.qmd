...
Use `match_data()` to extract the matches from the output of your call to `matchit()`. Among the matches, summarize the unweighted mean value of `y` within groups defined by `treated`. If you need help with this look back at where you estimated subgroup means in [Problem Set 2](pset2.qmd#summarize-the-data).
