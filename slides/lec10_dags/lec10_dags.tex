...
\begin{frame}{Causal path}{$\bullet\rightarrow\bullet\rightarrow\bullet$}

A path in which arrows point in the same direction \vskip .2in \pause
...
\begin{frame}{Causal path: Marginal dependence}{$\bullet\rightarrow\bullet\rightarrow\bullet$}
...
\begin{frame}{Causal path: Conditional independence}{$\bullet\rightarrow\bullet\rightarrow\bullet$}
...
\begin{frame}{Fork structure: Marginal dependence}{$\bullet\leftarrow\bullet\rightarrow\bullet$}
...
\begin{frame}[t]{Determining statistical dependence: A procedure}

How do you know if two nodes are dependent?
\begin{enumerate}
...
Practice: Are $A$ and \only<2-6>{$C$}\only<7-10>{$D$} statistically independent or dependent?
...
\only<3-6>{
\begin{itemize}
\item \only<4-6>{\st}{$A\rightarrow B \leftarrow C$}
\item \only<5-6>{\st}{$A\leftarrow X \rightarrow D \leftarrow C$}
\end{itemize}
\only<6>{No unblocked paths. $A$ and $C$ are independent!}
}
\only<8-10>{
\begin{itemize}
\item \only<9-10>{\st}{$A\rightarrow B \leftarrow C \rightarrow D$}
\item $A\leftarrow X \rightarrow D$
\end{itemize}
\only<10>{One remains. $A$ and $D$ are dependent!}
}
...
Practice: Are $A$ and $D$ statistically independent or dependent, conditional on $X$ and $B$?
