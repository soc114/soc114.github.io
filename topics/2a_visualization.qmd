...
This code starts with the dataset `incomeSimulated`. The pipe operator `|>` passes this object as the first argument to the next line (the `data` argument of ``ggplot`). The `ggplot()` line creates the plot. The `mapping` argument defines the aethetics (`aes`) of the plot, with the `hhincome` variable being assigned to the `x`-axis. The `geom_histogram` line produces the plot. The `scale_x_continuous()` lines are an optional part to improve readability of the x-axis.
