...
p <- sim_control |>
  ggplot(aes(x = x, y = y)) +
  ylab("Confidence in\nData Science\nAfter This Quarter") +
  xlab("Past Interest in Data Science") +
  xlim(c(0,1)) +
  ylim(c(0,1)) +
  theme_bw()
...
p +
  geom_point(data = sim_treated, aes(y = 1), color = "#2774AE") +
